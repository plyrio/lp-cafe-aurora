            ...
            <div className="text-center relative h-full">
                ...
                <div>
                    ...
                    >
                        <CarouselContent>
                            {feedbacks.map((item) => (
                                <CarouselItem key={item.id} className='flex mx-auto items-center justify-center lg:basis-1/2 xl:basis-1/3'>
                                    <div className="p-1 pb-20 h-full w-full">
                                        <Card className='bg-orange-50 h-full hover:bg-orange-100 border border-orange-100 flex md:p-8 flex-col shadow-none transition-shadow duration-300 cursor-pointer hover:shadow-lg hover:shadow-gray-400'>
                                            <img src={Aspas} alt="" className='max-h-10 ms-5 w-auto object-contain object-left ' />
                                            <CardContent className=" flex flex-col h-full aspect-auto items-center flex-1 px-12">
                                                <span className="text-base text-pretty text-neutral-500 sm:text-lg/relaxed">
                                                    {item.text}
                                                </span>

                                                <div className="mt-auto flex w-fit mx-auto flex-col items-center text-center gap-3 translate-y-[50%]">
                                                    <div className="flex flex-col ">
                                                        <h3 className="md:text-2xl text-xl text-yellow-900 font-aurorabold">
                                                            {item.name}
                                                        </h3>
                                                        <h5 className="md:text-xl text-sm text-neutral-500 sm:text-xl/relaxed">
                                                            {item.role}
                                                        </h5>
                                                    </div>
